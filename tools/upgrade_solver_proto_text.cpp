...
// Usage: upgrade_solver_proto_text old_solver_proto_file_in solver_proto_file_out
#include <cstring>
...
int main(int argc, char** argv) {
  ...
  if (argc != 3) {
    LOG(ERROR) << "Usage: upgrade_solver_proto_text old_solver_proto_file_in solver_proto_file_out";
    return 1;
  }
  ...
  if (!ReadProtoFromTextFile(input_filename, &solver_param)) {
    LOG(ERROR) << "Failed to parse input text file as SolverParameter: " << input_filename;
    return 2;
  }
