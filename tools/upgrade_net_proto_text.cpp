...
// Usage: upgrade_net_proto_text v0_net_proto_file_in net_proto_file_out
#include <cstring>
...
int main(int argc, char** argv) {
  ...
  if (argc != 3) {
    LOG(ERROR) << "Usage: upgrade_net_proto_text v0_net_proto_file_in net_proto_file_out";
    return 1;
  }
  ...
  if (!ReadProtoFromTextFile(input_filename, &net_param)) {
    LOG(ERROR) << "Failed to parse input text file as NetParameter: " << input_filename;
    return 2;
  }
