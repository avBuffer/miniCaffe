...
/* @brief Computes @f$ y = (\alpha x + \beta) ^ \gamma @f$, as specified by the scale @f$ \alpha @f$, 
 *        shift @f$ \beta @f$, and power @f$ \gamma @f$. */
template <typename Dtype>
class PowerLayer : public NeuronLayer<Dtype> {
 public:
  /* @param param provides PowerParameter power_param, with PowerLayer options:
   *   - scale (\b optional, default 1) the scale @f$ \alpha @f$
   *   - shift (\b optional, default 0) the shift @f$ \beta @f$
   *   - power (\b optional, default 1) the power @f$ \gamma @f$ */
  explicit PowerLayer(const LayerParameter& param) : NeuronLayer<Dtype>(param) {}
 ...
 protected:
   ...
   *   -# @f$ (N \times C \times H \times W) @f$ the computed outputs @f$ y = (\alpha x + \beta) ^ \gamma @f$ */
   ...
   *            \alpha \gamma (\alpha x + \beta) ^ {\gamma - 1} = \frac{\partial E}{\partial y}
