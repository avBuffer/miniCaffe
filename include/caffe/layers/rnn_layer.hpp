...
/* @brief Processes time-varying inputs using a simple recurrent neural network (RNN).
 *        Implemented as a network unrolling the RNN computation in time.
 * Given time-varying inputs @f$ x_t @f$, computes hidden state @f$
 *     h_t := \tanh[ W_{hh} h_{t_1} + W_{xh} x_t + b_h ]
 * @f$, and outputs @f$ o_t := \tanh[ W_{ho} h_t + b_o ] @f$. */
template <typename Dtype>
class RNNLayer : public RecurrentLayer<Dtype> {
 public:
  explicit RNNLayer(const LayerParameter& param) : RecurrentLayer<Dtype>(param) {}
