 ...
 *        Equivalent to tiling the latter Blob, then computing the elementwise product. 
 *        Note: for efficiency and convenience, this layer can
 *        additionally perform a "broadcast" sum too when `bias_term: true` is set.
 * The latter, scale input may be omitted, in which case it's learned as parameter of the layer
 * (as is the bias, if it is included). */
...
class ScaleLayer: public Layer<Dtype> {
 ...
 protected:
   ...
   * canonicalization (i.e., conversion from negative to positive index, if applicable).
   *
   * @param bottom input Blob vector (length 2)
   *   -# @f$ (d_0 \times ... \times d_i \times ... \times d_j \times ... \times d_n) @f$ the first factor @f$ x @f$
   *   -# @f$ (d_i \times ... \times d_j) @f$ the second factor @f$ y @f$
   * @param top output Blob vector (length 1)
   *   -# @f$ (d_0 \times ... \times d_i \times ... \times d_j \times ... \times d_n) @f$
   *      the product @f$ z = x y @f$ computed after "broadcasting" y.
   *      Equivalent to tiling @f$ y @f$ to have the same shape as @f$ x @f$, then computing the elementwise product. */
