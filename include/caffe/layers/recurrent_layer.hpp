...
/* @brief An abstract class for implementing recurrent behavior inside of an unrolled network.
 *        This Layer type cannot be instantiated -- instead, you should use one of its implementations
 *        which defines the recurrent architecture, such as RNNLayer or LSTMLayer. */
...
class RecurrentLayer : public Layer<Dtype> {
 ...
 protected:
   ...
   *        Subclasses should define this -- see RNNLayer and LSTMLayer for examples. */
  virtual void FillUnrolledNet(NetParameter* net_param) const = 0;

  /* @brief Fills names with the names of the 0th timestep recurrent input Blob&s.
   *        Subclasses should define this -- see RNNLayer and LSTMLayer for examples. */
  ...
  /* @brief Fills names with the names of the Tth timestep recurrent output Blob&s. 
   *        Subclasses should define this -- see RNNLayer and LSTMLayer for examples. */
  virtual void RecurrentOutputBlobNames(vector<string>* names) const = 0;

  /* @brief Fills names with the names of the output blobs, concatenated across all timesteps. 
   *        Should return a name for each top Blob.
   *        Subclasses should define this -- see RNNLayer and LSTMLayer for examples. */
  virtual void OutputBlobNames(vector<string>* names) const = 0;

  /* @param bottom input Blob vector (length 2-3)
   *   -# @f$ (T \times N \times ...) @f$ the time-varying input @f$ x @f$.
   *      After the first two axes, whose dimensions must correspond to the number of timesteps @f$ T @f$ 
   *      and the number of independent streams @f$ N @f$, respectively, its dimensions may be arbitrary.
   *      Note that the ordering of dimensions -- @f$ (T \times N \times ...) @f$, rather than
   *      @f$ (N \times T \times ...) @f$ -- means that the @f$ N @f$ independent input streams must be "interleaved".
   
   *   -# @f$ (T \times N) @f$ the sequence continuation indicators @f$ \delta @f$.
   ...
   *      @f$ t-1 @f$, and the previous hidden state @f$ h_{t-1} @f$ 
   *      affects the updated hidden state and output.
   ...
   *      single batch.  This may require padding and/or truncation for uniform length.

   * @param top output Blob vector (length 1)
   *   -# @f$ (T \times N \times D) @f$
   *      the time-varying output @f$ y @f$, where @f$ D @f$ is <code>recurrent_param.num_output()</code>.
  ...
  int N_;
  /* @brief The number of timesteps in the layer's input, and the number of timesteps over which to backpropagate through time. */
  int T_;
  /// @brief Whether the layer has a "static" input copied across all timesteps.
  bool static_input_;
  /* @brief The last layer to run in the network. (Any later layers are losses added to force the recurrent net to do backprop.) */
  int last_layer_index_;
  /* @brief Whether the layer's hidden state at the first and last timesteps are layer inputs and outputs, respectively. */
  bool expose_hidden_;
