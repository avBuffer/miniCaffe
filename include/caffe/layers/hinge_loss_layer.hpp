 ...
 *   -# @f$ (N \times C \times H \times W) @f$ the predictions @f$ t @f$, a Blob with values in
 ...
 *      HingeLossLayer and no other learnable parameters or losses is equivalent to an SVM.
 ...
 *      the computed hinge loss: @f$ E =\frac{1}{N} \sum\limits_{n=1}^N \sum\limits_{k=1}^K
 *        [\max(0, 1 - \delta\{l_n = k\} t_{nk})] ^ p
 *      @f$, for the @f$ L^p @f$ norm (defaults to @f$ p = 1 @f$, the L1 norm; L2 norm, as in L2-SVM,
 *      is also available), and @f$
 *      \delta\{\mathrm{condition}\} = \left\{
 *         \begin{array}{lr}
 *            1 & \mbox{if condition} \\ -1 & \mbox{otherwise}
 *         \end{array} \right. @f$
 * In an SVM, @f$ t \in \mathcal{R}^{N \times K} @f$ is the result of taking the inner product @f$ X^T W @f$ of the features
 * @f$ X \in \mathcal{R}^{D \times N} @f$ and the learned hyperplane parameters
...
class HingeLossLayer : public LossLayer<Dtype> {
 public:
  explicit HingeLossLayer(const LayerParameter& param) : LossLayer<Dtype>(param) {}
 ...
 protected:
   ...
   * @param bottom input Blob vector (length 2)
   *   -# @f$ (N \times C \times H \times W) @f$ the predictions @f$t@f$; Backward computes diff @f$ \frac{\partial E}{\partial t} @f$
