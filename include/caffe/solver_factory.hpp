/* @brief A solver factory that allows one to register solvers, similar to layer factory.
 * During runtime, registered solvers could be called by passing a SolverParameter protobuffer to the CreateSolver function:
 ...
 *      // your implementation
 *    }
 * then you can register the creator function instead, like REGISTER_SOLVER_CREATOR(MyAwesome, GetMyAwesomeSolver)
...
class SolverRegistry {
 public:
  ...
  static Solver<Dtype>* CreateSolver(const SolverParameter& param) {
    ...
    CreatorRegistry& registry = Registry();
    CHECK_EQ(registry.count(type), 1) << "Unknown solver type: " << type << " (known types: " << SolverTypeListString() << ")";
