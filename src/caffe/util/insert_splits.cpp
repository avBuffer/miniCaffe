  ...
  for (int i = 0; i < param.layer_size(); ++i) {
    ...
    for (int j = 0; j < layer_param.bottom_size(); ++j) {
      ...
      if (blob_name_to_last_top_idx.find(blob_name) == blob_name_to_last_top_idx.end()) {
        LOG(FATAL) << "Unknown bottom blob '" << blob_name << "' (layer '" << layer_param.name() << "', bottom index " << j << ")";
      }
    ...
    // A use of a top blob as a loss should be handled similarly to the use of a top blob as a bottom blob to another layer.
  ...
  for (int i = 0; i < param.layer_size(); ++i) {
    ...
    for (int j = 0; j < layer_param->bottom_size(); ++j) {
      ...
      if (split_count > 1) {
        ...
        const string& blob_name = layer_param->bottom(j);
        layer_param->set_bottom(j, SplitBlobName(layer_name, blob_name, top_idx.second, top_idx_to_bottom_split_idx[top_idx]++));
      }
    }
...
void ConfigureSplitLayer(const string& layer_name, const string& blob_name, const int blob_idx, const int split_count,
                         const float loss_weight, LayerParameter* split_layer_param) {
...
string SplitBlobName(const string& layer_name, const string& blob_name, const int blob_idx, const int split_idx) {
  ...
  split_blob_name << blob_name << "_" << layer_name << "_" << blob_idx << "_split_" << split_idx;
