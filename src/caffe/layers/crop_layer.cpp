...
template <typename Dtype>
void CropLayer<Dtype>::crop_copy(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top, const int* offsets,
...
void CropLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  crop_copy(bottom, top, offsets.cpu_data(), indices, 0, bottom_data, top_data, true);
}
...
void CropLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, 
  ...
  if (propagate_down[0]) {
    ...
    std::vector<int> indices(top[0]->num_axes(), 0);
    crop_copy(bottom, top, offsets.cpu_data(), indices, 0, top_diff, bottom_diff, false);
  }
}
