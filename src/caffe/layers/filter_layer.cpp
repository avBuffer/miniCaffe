...
void FilterLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void FilterLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  for (int i = 1; i < bottom[selector_index]->num_axes(); ++i) {
    CHECK_EQ(bottom[selector_index]->shape(i), 1) << "Selector blob dimensions must be singletons (1), except the first";
  }
  ...
  // have the same index as the items in bottom[0] with value == non-zero will be forwarded
...
template <typename Dtype>
void FilterLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  int new_tops_num = indices_to_forward_.size();
...
template <typename Dtype>
void FilterLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, 
    ...
    if (propagate_down[i]) {
      ...
      for (int n = 0; n < bottom[i]->shape(0); n++) {
        ...
        if (next_to_backward_offset >= indices_to_forward_.size()) {
          ...
          caffe_set(dim, Dtype(0), bottom[i]->mutable_cpu_diff() + data_offset_bottom);
        } else {
          ...
          if (n != batch_offset) {  // this data was not been forwarded
            ...
            caffe_copy(dim, top[i]->mutable_cpu_diff() + data_offset_top, bottom[i]->mutable_cpu_diff() + data_offset_bottom);
          }
        }
      }
    }
