...
void ReshapeLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  for (int i = 0; i < top_num_axes; ++i) {
    ...
    if (top_dim == 0) {
      ...
      CHECK_EQ(inferred_axis_, -1) << "new shape contains multiple -1 dims; at most a single (1) value of -1 may be specified";
      inferred_axis_ = i;
    } else {
...
void ReshapeLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  CHECK_LE(start_axis, bottom[0]->num_axes()) << "axis " << input_start_axis << " out of range for " << bottom[0]->num_axes() << "-D input blob";
  ...
  if (inferred_axis_ >= 0) {
    // A -1 dim was specified; infer the correct dimension by computing the product of the other dimensions.
    ...
    CHECK_EQ(0, bottom[0]->count() % explicit_count) << "bottom count (" << bottom[0]->count() << 
        ") must be divisible by the product of the specified dimensions (" << explicit_count << ")";
