...
template <typename Dtype>
void PowerLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down,
  ...
  if (propagate_down[0]) {
    ...
    if (diff_scale_ == Dtype(0) || power_ == Dtype(1)) {
      ...
      const Dtype* bottom_data = bottom[0]->cpu_data();
      // Compute dy/dx = scale * power * (shift + scale * x)^(power - 1) = diff_scale * y / (shift + scale * x)
      if (power_ == Dtype(2)) {
        // Special case for y = (shift + scale * x)^2
        //     -> dy/dx = 2 * scale * (shift + scale * x) = diff_scale * shift + diff_scale * scale * x
        ...
        //     -> dy/dx = scale * power * (scale * x)^(power - 1) = scale * power * (scale * x)^power * (scale * x)^(-1)
        //              = power * y / x
