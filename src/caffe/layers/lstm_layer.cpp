...
void LSTMLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  ...
  // Add layer to transform all timesteps of x to the hidden state dimension. W_xc_x = W_xc * x + b_c
  {
  ...
  if (this->static_input_) {
    // Add layer to transform x_static to the gate dimension. W_xc_x_static = W_xc_static * x_static
  ...
  for (int t = 1; t <= this->T_; ++t) {
    ...
    // Add layers to flush the hidden state when beginning a new sequence, as indicated by cont_t.
    ...
    // Add layer to compute W_hc_h_{t-1} := W_hc * h_conted_{t-1}
    {
    ...
    //     gate_input_t := W_hc * h_conted_{t-1} + W_xc * x_t + b_c = W_hc_h_{t-1} + W_xc_x_t + b_c
    {
