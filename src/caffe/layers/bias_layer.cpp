  ...
  if (bottom.size() == 1 && this->blobs_.size() > 0) {
    ...
    if (num_axes >= 0) {
      CHECK_GE(bottom[0]->num_axes(), axis + num_axes) <<
          "bias blob's shape extends past bottom[0]'s shape when applied starting with bottom[0] axis = " << axis;
    }

    this->blobs_.resize(1);
    const vector<int>::const_iterator& shape_start = bottom[0]->shape().begin() + axis;
...
void BiasLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  // Always set axis == 0 in special case where bias is a scalar (num_axes == 0). 
  // Mathematically equivalent for any choice of axis, so the actual setting can be safely ignored;
  // and computation is most efficient with axis == 0 and (therefore) outer_dim_ == 1.
  ...
  for (int i = 0; i < bias->num_axes(); ++i) {
    CHECK_EQ(bottom[0]->shape(axis + i), bias->shape(i)) <<
        "dimension mismatch between bottom[0]->shape(" << axis + i << ") and bias->shape(" << i << ")";
  }
...
template <typename Dtype>
void BiasLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  for (int n = 0; n < outer_dim_; ++n) {
    caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, bias_dim_, inner_dim_, 1, Dtype(1), bias_data,
...
template <typename Dtype>
void BiasLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down,
  ...
  if ((!bias_param && propagate_down[1]) || (bias_param && this->param_propagate_down_[0])) {
    ...
    for (int n = 0; n < outer_dim_; ++n) {
      caffe_cpu_gemv(CblasNoTrans, bias_dim_, inner_dim_, Dtype(1), top_diff, bias_multiplier_.cpu_data(), Dtype(accum), bias_diff);
      top_diff += dim_;
      accum = true;
    }
  }
