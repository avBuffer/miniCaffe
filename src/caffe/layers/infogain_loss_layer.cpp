...
void InfogainLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  // normalization
  CHECK(!this->layer_param_.loss_param().has_normalize()) << "normalize is deprecated. use \"normalization\"";
...
void InfogainLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  CHECK_EQ(outer_num_ * inner_num_, bottom[1]->count()) <<
      "Number of labels must match number of predictions; e.g., if infogain axis == 1 and prediction shape is (N, C, H, W), " <<
...
template <typename Dtype>
Dtype InfogainLossLayer<Dtype>::get_normalizer(LossParameter_NormalizationMode normalization_mode, int valid_count) {
  Dtype normalizer;
...
void InfogainLossLayer<Dtype>::sum_rows_of_H(const Blob<Dtype>* H) {
  CHECK_EQ(H->count(), num_labels_*num_labels_) << "H must be " << num_labels_ << "x" << num_labels_;
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      for (int l = 0; l < num_labels_; l++) {
        loss -= infogain_mat[label_value * num_labels_ + l] * log(std::max(prob_data[i * inner_num_*num_labels_ + l * inner_num_ + j], Dtype(kLOG_THRESHOLD)));
      }
      ++count;
    }
  }
        ...
        if (has_ignore_label_ && label_value == ignore_label_) {
          ...
          for (int l = 0; l < num_labels_; ++l) {
            bottom_diff[i * dim + l * inner_num_ + j] =
                prob_data[i * dim + l * inner_num_ + j] * sum_rows_H[label_value] - infogain_mat[label_value * num_labels_ + l];
          }
          ++count;
        }
      }
