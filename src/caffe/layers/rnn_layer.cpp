...
void RNNLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  ...
  const FillerParameter& weight_filler = this->layer_param_.recurrent_param().weight_filler();
  const FillerParameter& bias_filler = this->layer_param_.recurrent_param().bias_filler();

  // Add generic LayerParameter's (without bottoms/tops) of layer types we'll use to save redundant code.
  ...
  // Add layer to transform all timesteps of x to the hidden state dimension. W_xh_x = W_xh * x + b_h
  {
  ...
  if (this->static_input_) {
    // Add layer to transform x_static to the hidden state dimension. W_xh_x_static = W_xh_static * x_static
    ...
    BlobShape* new_shape = reshape_param->mutable_reshape_param()->mutable_shape();
    new_shape->add_dim(1);  // One timestep.
    // Should infer this->N as the dimension so we can reshape on batch size.
    new_shape->add_dim(-1);
    new_shape->add_dim(x_static_transform_param->inner_product_param().num_output());
  ...
  for (int t = 1; t <= this->T_; ++t) {
    ...
    // Add layer to flush the hidden state when beginning a new sequence, as indicated by cont_t. h_conted_{t-1} := cont_t * h_{t-1}
    // Normally, cont_t is binary (i.e., 0 or 1), so: h_conted_{t-1} := h_{t-1} if cont_t == 1,  0   otherwise
    {
    ...
    // Add layer to compute W_hh_h_{t-1} := W_hh * h_conted_{t-1}
    {
    ...
    // Add layers to compute h_t := \tanh( W_hh * h_conted_{t-1} + W_xh * x_t + b_h ) = \tanh( W_hh_h_{t-1} + W_xh_t )
    {
    ...
    // Add layer to compute W_ho_h_t := W_ho * h_t + b_o
    {
      LayerParameter* w_param = net_param->add_layer();
    ...
    // Add layers to compute o_t := \tanh( W_ho * h_t + b_o) = \tanh( W_ho_h_t )
    {
